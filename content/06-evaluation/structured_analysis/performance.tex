...
In performance across tags, we cannot observe any obvious patterns, as \ac{cv} and \ac{pv} did not struggle with the same topics. What can be noted is a more pronounced difference in entity\_identification scores. Those exercises required more patience, since they involved reading a textual description. That may hint that participants in \ac{cv} were less willing to rush exercises out of impatience. 

For both groups the code\_vs\_diagram category was in the top 2 places score-wise (excluding the difficulty markers). That supports the findings of other researchers that students understand diagrams better when linked to code.

% We can observe that apart from difficulty labels, relationships

% Fluctuations in performance across categories are likely to a large extent caused by exercise design. However, we can
