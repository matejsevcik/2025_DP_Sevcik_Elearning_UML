...
We can see that students with lower Intro scores tended to improve more significantly.
...
When comparing performance in individual exercises, \ac{pv} only scores better in 5 out of 28. That is a significant difference, which might not be attributed to a slightly higher baseline knowledge alone.
...
% The objective performance is higher in \ac{cv}, while relative improvement is higher in \ac{pv}. This can be explained mainly by the fact that \ac{cv} has a higher baseline skill, which also reduces the potential for improvement in its participants. However, the results show very consistent
